...
import type { BotAnswer } from "@/modules/bot/domain/types";
...
export async function generateAnswer(userMessage: string): Promise<BotAnswer> {
  ...
  if (!retrieval.hasGroundedContext) {
    return { type: "escalate", question: userMessage };
  }
  ...
  if (toolCall && toolCall.type === "function") {
    ...
    if (parsed.success) {
      return { type: "escalate", question: parsed.data.question, email: parsed.data.email };
    }
    return { type: "escalate", question: userMessage };
  }
